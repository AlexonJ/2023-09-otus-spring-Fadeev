    ...
    public ItemProcessor<Book, ru.otus.spring.bookstore.models.nonrelational.Book> bookProcessor(MongoTemplate mongoTemplate) {
        return book -> {
            ...
            var authorMongo = Optional.ofNullable(authorRelations.get(author.getId())).map(authorId -> {
                ...
                return mongoTemplate.findOne(query, ru.otus.spring.bookstore.models.nonrelational.Author.class, AUTHORS_COLLECTION_NAME);
//                mongoTemplate.findAll(ru.otus.spring.bookstore.models.nonrelational.Author.class, AUTHORS_COLLECTION_NAME)
            }).orElseThrow();
